...
class AtomSelectorSpec extends AnyFlatSpec with Matchers:
  ...
  it should "not extract types that aren't there" in:
    illTyped("AtomSelector[Int *: String *: EmptyTuple, Boolean]")

  it should "work on simple hlists" in:
    val list = 5 *: "hello" *: EmptyTuple
    AtomSelector[Int *: String *: EmptyTuple, Int].apply(list) shouldBe 5
    AtomSelector[Int *: String *: EmptyTuple, String].apply(list) shouldBe "hello"
  ...
  it should "work on (A, B) *: (C, D) *: EmptyTuple" in:
    implicit val pairAtom: Atom[Pair] = Atom[Pair]
    type T = (Int, String) *: (Char, Boolean) *: EmptyTuple
    val t = (1, "one") *: ('1', true) *: EmptyTuple
